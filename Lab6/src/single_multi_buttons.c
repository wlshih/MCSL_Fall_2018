...
int main() {
	...
	while(1) {
		input = keypad_scan();
		if(input == -1) display_num = 0;
		else if((display_num + input) <= 99999999) display_num += input;
...
int keypad_scan() {
	...
	int table[4][4] = {{1, 4, 7, -1}, {2, 5, 8, 0}, {3, 6, 9, -1}, {10, 11, 12, 13}};
	...
	while(1) {
		...
		if(flag_keypad != 0){ // detected input -> debounce
			...
			if(flag_debounce != 0) {
				// check from rows to cols
				for(j=0; j<4; j++) { // read each keypad rows (input)
					for(i=0; i<4; i++) { // scan each keypad columns (output)
